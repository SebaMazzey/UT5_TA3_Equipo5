...
public class TElementoAB<T> implements IElementoAB<T> {
    ...
    public int calcularCosto(int[] FrecExito, int[] FrecNoExito, int[] indiceFE, int[] indiceFNE, int nivel) {
        ...
        int suma = 0;
        if (hijoIzq != null) {
            suma = suma + hijoIzq.calcularCosto(FrecExito, FrecNoExito, indiceFE, indiceFNE, nivel);
        } else {
            suma = suma + (nivel + 1) * (FrecNoExito[indiceFNE[0]]);
            indiceFNE[0]++;
        }
        indiceFE[0]++;
        suma = suma + nivel * FrecExito[indiceFE[0]];

        if (hijoDer != null) {
            suma = suma + hijoDer.calcularCosto(FrecExito, FrecNoExito, indiceFE, indiceFNE, nivel);
        } else {
            suma = suma + (nivel + 1) * (FrecNoExito[indiceFNE[0]]);
            indiceFNE[0]++;
        }
